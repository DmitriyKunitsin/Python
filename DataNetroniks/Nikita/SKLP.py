...
class SKLP:
    ...
    def split_float(Packet, value):
        hex_value = str(SKLP.float_to_hex(value))
        
        Packet.append( int(hex_value[2:4], 16) )
        Packet.append( int(hex_value[4:6], 16) )
        Packet.append( int(hex_value[6:8], 16) )
        Packet.append( int(hex_value[8:10], 16) )
    
    def NVM_Write_Datetime_Now(Packet):
        time = str(datetime.datetime.now())
        Packet.append(int(time[2:4]))
        Packet.append(int(time[5:7]))
        Packet.append(int(time[8:10]))
        Packet.append(int(time[11:13]))
        Packet.append(int(time[14:16]))
        Packet.append(int(time[17:19]))
...
class SKLP_Serial( SKLP ):
    ...
    def PrintListCOM():
        ...
        for ( port, desc, hwid ) in ports:
            all_ports = "{}:\t{:30}\t{}".format( port, desc, hwid )
            print( all_ports )
...
class SKLP_Module_RUS_Regul( SKLP_Module ):
    class Commands:
        ...
        NVM_Get             = 0x04 #? Прочитать данные в NVM
        ...
        SyncResultGet       = 0x66 #? Отдать результаты калибровок
        RangeAdjustment     = 0x67 #? Записать корректировку диапазона датчика давленияы
    ...
    class Parse:
        ...
        NVM_Get_Press       = '=7B3f'
        NVM_Get_Motor       = '=7B2f'
        SyncProcessResult   = '=HHhhhh'                 #? Результат калибровок - ответ на команду 0х66
        RangeAdjCoefAns     = '=h'                      #? Коэффициент корректировки диапазона ДД
        ExtDataGet          = '=BHBbhHHHBBlhhhHBB'      #? Полный ответ на 14 команду
        ...
        ExtDataGetSyncData  = '=HHhhHh'                 #? Блок данных синхронизации из 14 команды
        ...
        OverlapSet          = '=B'                      #? Параметр команды 62 - выставить процент перекрытия
        RangeAdjCoefQuery   = '=Bf'                     #? Параметр команды 67 - корректировка диапазона ДД
        StabilizePress      = '=f'                      #? Параметр команды 72 - выставить скорость
        Eeprom_Press_Coefs  = '=7B3fH'                  #? Записать коэффициенты для калибровки ДД в NVM
        Eeprom_Motor_Coefs  = '=7B2fH'                  #? Записать макисмальный ток и максимальную скорость мотора в NVM
    ...
    def __init__(self, Address, Interface, Name ='Regul'):
        ...
        self.NVM_Pressure   = SKLP_Module_RUS_Regul.NVM_Pressure()
        self.NVM_Motor      = SKLP_Module_RUS_Regul.NVM_Motor()
        self.WorkMode       = 0
    ...
    def Query_ExDataGet(self, Timeout = 0.2):
        ...
        if( ( AnswerPacket != None ) and ( len( AnswerPacket ) == PacketSize ) ):
            ...
            self.PressData.ADCCode, self.PressData.Instant,
            self.PressData.Average, self.PressData.ShiftPress, self.PressData.Target,

            self.PowerSupply, self.WorkMode) = unpack(self.Parse.ExtDataGet, AnswerPacket)
    ...
    #! Формально такая функция есть, и мотор даже выполнит калибровку (Может быть!!!), только никто не планирует использовать это в скважине
    ...
    def Query_NVM_Set(self, NVM_ID, data):
        Packet_for_CRC16 = [] # Пакет для расчета CRC16 (отличие в преобразовании float к hex (4 байта))
        
        
        if(NVM_ID == 0):
            SKLP.NVM_Write_Datetime_Now(Packet_for_CRC16)
            
            C2, C1, C0 = data
            
            SKLP.split_float(Packet_for_CRC16, C2)
            SKLP.split_float(Packet_for_CRC16, C1)
            SKLP.split_float(Packet_for_CRC16, C0)
            
            CRC16 = SKLP.Calc_CRC16(Packet_for_CRC16, len(Packet_for_CRC16), 0)
            self.Query(self.Commands.NVM_Set, pack(self.Parse.Eeprom_Press_Coefs, NVM_ID, Packet_for_CRC16[0], Packet_for_CRC16[1], 
            Packet_for_CRC16[2], Packet_for_CRC16[3], Packet_for_CRC16[4], Packet_for_CRC16[5], C2, C1, C0, CRC16))
            
        elif(NVM_ID == 1):
            MaxCurrent, MaxSpeed = data
            
            SKLP.NVM_Write_Datetime_Now(Packet_for_CRC16)
            
            SKLP.split_float(Packet_for_CRC16, MaxCurrent)
            SKLP.split_float(Packet_for_CRC16, MaxSpeed)
            
            CRC16 = SKLP.Calc_CRC16(Packet_for_CRC16, len(Packet_for_CRC16), 0)
            self.Query(self.Commands.NVM_Set, pack(self.Parse.Eeprom_Motor_Coefs, NVM_ID, Packet_for_CRC16[0], Packet_for_CRC16[1], 
            Packet_for_CRC16[2], Packet_for_CRC16[3], Packet_for_CRC16[4], Packet_for_CRC16[5], MaxCurrent, MaxSpeed, CRC16))
        
    
    def Query_NVM_Get(self, NVM_ID):
        AnswerPacket = self.Query(self.Commands.NVM_Get, pack(self.Parse.NVM_ID, NVM_ID))
        if(NVM_ID == 0x00):    
            PacketSize = calcsize(self.Parse.NVM_Get_Press)
            if( ( AnswerPacket != None ) and ( len( AnswerPacket ) == PacketSize ) ):
                (self.NVM_Pressure.NVM_Tag, self.NVM_Pressure.Year, self.NVM_Pressure.Month, self.NVM_Pressure.Day,
                self.NVM_Pressure.Hour, self.NVM_Pressure.Minute, self.NVM_Pressure.Second,
                self.NVM_Pressure.Coef_C2, self.NVM_Pressure.Coef_C1, self.NVM_Pressure.Coef_C0) = unpack(self.Parse.NVM_Get_Press, AnswerPacket)
                 
                print(f'Давление:\n {self.NVM_Pressure.Year}\tГод\n {self.NVM_Pressure.Month}\tМесяц\n {self.NVM_Pressure.Day}\tДень\n {self.NVM_Pressure.Hour}\tЧас\n {self.NVM_Pressure.Minute}\tМинута\n {self.NVM_Pressure.Second}\tСекунда\n {self.NVM_Pressure.Coef_C2}\tКоэф С2\n {self.NVM_Pressure.Coef_C1}\tКоэф С1\n {self.NVM_Pressure.Coef_C0}\tКоэф С0\n')
                
        elif(NVM_ID == 0x01):
            PacketSize = calcsize(self.Parse.NVM_Get_Motor)
            if( ( AnswerPacket != None ) and ( len( AnswerPacket ) == PacketSize ) ):
                (self.NVM_Motor.NVM_Tag, self.NVM_Motor.Year, self.NVM_Motor.Month, self.NVM_Motor.Day, self.NVM_Motor.Hour,
                self.NVM_Motor.Minute, self.NVM_Motor.Second, self.NVM_Motor.MaxCurrent, self.NVM_Motor.MaxSpeed) = unpack(self.Parse.NVM_Get_Motor, AnswerPacket)
            
            print(f'Блок мотора:\n {self.NVM_Motor.Year}\tГод\n {self.NVM_Motor.Month}\tМесяц\n {self.NVM_Motor.Day}\tДень\n {self.NVM_Motor.Hour}\tЧас\n {self.NVM_Motor.Minute}\tМинута\n {self.NVM_Motor.Second}\tСекунда\n {self.NVM_Motor.MaxCurrent}\tМакисмальный ток\n {self.NVM_Motor.MaxSpeed}\tМаксимальная скорость')

                
        else:
            print("No Answer!")
        
    def Query_SyncResultGet(self, Timeout = 0.2):
        AnswerPacket = self.Query(self.Commands.SyncResultGet, Timeout = Timeout)
        PacketSize = calcsize(self.Parse.SyncProcessResult)
        if( ( AnswerPacket != None ) and ( len( AnswerPacket ) == PacketSize ) ):
            (self.SyncData.MinAngle, self.SyncData.MaxAngle,
            self.SyncData.MinPress, self.SyncData.MaxPress,
            self.SyncData.WorkZone, self.SyncData.AdjCoef) = unpack(self.Parse.SyncProcessResult, AnswerPacket)
            ...
            print("No answer on 0x66 command!")
    
    def Query_RangeAdjustment(self, param, Coef, timeout = 0.2):
        AnswerPacket = self.Query(self.Commands.RangeAdjustment, pack(self.Parse.RangeAdjCoefQuery, param, Coef), Timeout = timeout)
        PacketSize = calcsize(self.Parse.RangeAdjCoefAns)
        if( ( AnswerPacket != None ) and ( len( AnswerPacket ) == PacketSize ) ):
            (self.PressData.RangeAdj) = unpack(self.Parse.RangeAdjCoefAns, AnswerPacket)
            
            self.PressData.RangeAdj = self.PressData.RangeAdj[0]
            self.PressData.RangeAdj /= 100
        else:
            print("No answer on 0x67 command!")
