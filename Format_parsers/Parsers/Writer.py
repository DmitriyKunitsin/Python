...
class FileSplitWriter:
    ...
    def handle_format(self, packet_format, iterator) -> str:
        """по формату пакета вызывает нужный обработчик
            return : возвращает разобранный пакет
        """
        hex_format = f'0x{packet_format:02x}'
        ...
        try:
            if handle:
                header = bytes(islice(iterator,5))
            ...
            print(f'Произошла ошибка в методе  {method_name} : {err} : искачал формат {hex_format} | {",".join(str(f'{x:02x}') for x in body)}')
    def get_size_packet(self, header : bytes) -> int:
        dev_id = header[0]
        token = header[1]
        number_digit_first = header[2:4]
        return header[4] 
    def payload_process_format(self ,payload:bytes, split_need : bool = False) -> str:
        ...
        try:
            ...
            for format_byte in iterator:
                ...
                if split_need and format_byte in valid_split:
                    encoding_packet = self.handle_format(format_byte, iterator)
